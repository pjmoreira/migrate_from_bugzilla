...
desc 'Fix a migration by copying data from one field to another'
...
namespace :redmine do
  task :copy_field_values => :environment do

    module CopyFieldValues
      ...
      def self.mass_update()
        ...
        for project_id in [77, 75, 74, 73, 72] #[82,83]
          ...
          for issue in Issue.find(:all, :conditions => {:project_id => project_id})
            ...
            if issue_custom_field.nil?
              ...
              if issue.fixed_version != nil and issue_custom_field.value == nil
                ...
                issue_custom_field.value = issue.fixed_version.nil??nil:issue.fixed_version.id
                # Status(8)=Developed && Status(10)=Completed
                if issue.status_id != 10 and issue.status_id != 8
      ...
      puts
      puts "REDMINE MASS UPDATE (for copying field values)"
      puts
